...
const classes = [
  { id: 1, title: "On-Camera Confidence Masterclass", topic: "Media", format: "Workshop", level: "Beginner", date: "March 22, 2026", price: "Free", spots: "Open", desc: "Build the foundational skills needed to perform confidently in front of any camera." },
  { id: 2, title: "Social Media Strategy Intensive", topic: "Digital", format: "Class", level: "Intermediate", date: "April 5, 2026", price: "$75", spots: "Open", desc: "Learn platform-specific strategies for growing and monetizing your online presence." },
  { id: 3, title: "Music Industry Fundamentals", topic: "Music", format: "Workshop", level: "Beginner", date: "April 12, 2026", price: "$50", spots: "Open", desc: "Understand contracts, royalties, distribution, and the business side of music." },
  { id: 4, title: "Advanced Interview Techniques", topic: "Media", format: "Masterclass", level: "Advanced", date: "April 19, 2026", price: "$120", spots: "Waitlist", desc: "Master the art of conducting and participating in high-stakes interviews." },
  { id: 5, title: "Personal Branding for Creatives", topic: "Branding", format: "Class", level: "Beginner", date: "May 3, 2026", price: "Free", spots: "Open", desc: "Define, develop, and communicate your personal brand identity." },
  { id: 6, title: "Public Speaking & Stage Presence", topic: "Performance", format: "Workshop", level: "Intermediate", date: "May 10, 2026", price: "$90", spots: "Open", desc: "Command any stage or podium with professional presence and delivery techniques." },
];
...
export default function ClassesWorkshops() {
  ...
  return (
    <Layout>
      <PageHero badge="Classes & Workshops" title="Learn. Practice. Grow." subtitle="Browse our upcoming classes, workshops, and masterclasses. Filter by topic, format, or experience level." />
                ...
                <motion.div key={c.id} {...fadeUp} transition={{ duration: 0.6, delay: i * 0.05 }} className="card-premium overflow-hidden card-hover flex flex-col">
                  <div className="p-6 flex-1">
                    ...
                    <div className="space-y-1.5 text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-2"><Users className="w-3.5 h-3.5" /> {c.spots === "Waitlist" ? <span className="text-primary">Waitlist</span> : "Spots available"}</div>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4" onClick={() => setSelectedClass(null)}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card-premium p-8 max-w-lg w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2 text-sm text-muted-foreground mb-6">
              ...
              <p><strong className="text-foreground">Price:</strong> {selectedClass.price}</p>
              <p><strong className="text-foreground">Status:</strong> {selectedClass.spots}</p>
            </div>
            <div className="flex gap-3">
              <Button variant="gold" asChild className="flex-1"><Link to="/enrollment-success">{selectedClass.spots === "Waitlist" ? "Join Waitlist" : "Enroll Now"}</Link></Button>
